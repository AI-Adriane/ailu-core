 ...
 * and run on the engine (Rust when the `@ailu-ai/napi` addon is present, else the
 ...
 *
 * Run it:
 *   pnpm --filter @ailu-ai/graph-sdk example:product
 *   pnpm --filter @ailu-ai/graph-sdk exec node --import tsx examples/product-pipeline.ts
 */
...
const main = async (): Promise<void> => {
  ...
  const engine = app.usesRustEngine ? "Rust (@ailu-ai/napi)" : "TypeScript (fallback)";
