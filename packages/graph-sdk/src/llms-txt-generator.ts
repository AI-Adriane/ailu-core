...
- \`app.approveAndResume(runId, { approvedTools, resolvedBy })\` — grant gated tools; \`resolvedBy\` (the human approver) is required.
...
\`AILU_MISSING_PROVIDER_KEY\`, \`AILU_NO_PROVIDER_IN_ENV\`, \`AILU_APPROVER_REQUIRED\`, \`AILU_APPROVAL_NOT_GRANTED\`.
...
- A governed resume (\`resumeCatalogGraph(definition, state, { approvalEngine })\`) refuses to continue until
  the engine has approved what the run waits on.
