...
```ts
import { model, createGraph } from "@ailu/graph-sdk";
...
- The per-provider packages (`@ailu/model-openai`, `-anthropic`, …) still exist (ADR 0031);
  `model.<provider>` is the unified front door over them.
- `@ailu/graph-sdk` installs **without** any provider SDK (ADR 0034 16a).
