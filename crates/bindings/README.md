...
Published to the pnpm workspace as `@ailu/napi` (see `pnpm-workspace.yaml`).
...
```bash
pnpm --filter @ailu/napi run build:napi          # napi build --platform --release
pnpm --filter @ailu/napi run build:napi:debug    # napi build --platform
```
...
requires the suffixed binary (or an `@ailu/napi-<platform>` sub-package), which
...
`packages/graph-sdk/src/rust-validator.ts` loads `@ailu/napi` **optionally**:
...
`@ailu/graph-core`. Nothing in the SDK hard-depends on the native addon.
