# ADR 0003 — Deprecate the TypeScript engine; run @ailu-ai/graph-sdk on Rust
...
gateway (`llm-gateway`), and the rest of the engine surface, and `@ailu-ai/graph-sdk`
has been re-plumbed to execute on them through a native addon (`@ailu-ai/napi`).
...
only as a fallback. `@ailu-ai/graph-sdk` remains the supported front door and runs on
...
- **`@ailu-ai/graph-sdk`** — the kept, supported front door. Its public API is
...
- **`@ailu-ai/graph-core`** — the pure data model + Zod validator. It is the shared
...
- `@ailu-ai/contracts`, `@ailu-ai/db`, `@ailu-ai/config`, `@ailu-ai/ui` — not engine
  execution packages; unaffected.

### Architecture: the napi async + ThreadsafeFunction bridge

`@ailu-ai/graph-sdk` reaches the Rust engine through the optional `@ailu-ai/napi`
...
> Run on **Rust if the `@ailu-ai/napi` addon is present; otherwise fall back to the
> (deprecated) TypeScript engine.**

When the addon is absent, `rustEngineAvailable()` returns `false` and `CompiledGraph`
executes on the deprecated `@ailu-ai/graph-runtime`. This keeps `main` building and the
...
- **Clear direction for consumers:** the IDE surfaces the `@deprecated` banners and
  ...
  `@ailu-ai/graph-sdk` and the Rust crates instead of the TS engine internals.
