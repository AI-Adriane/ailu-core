...
- **Engine** — new pure package `@ailu-ai/search`: the `SearchProvider` interface, the wire
...
- Search is eventually-consistent with Postgres (best-effort indexing); `POST /search/reindex`
  ...
  reindex.
- New engine public API (`@ailu-ai/search`) — additive, no breaking change to existing
  packages.
